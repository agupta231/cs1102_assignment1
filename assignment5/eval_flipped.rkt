...
(define (minmax_ok state)
  (local [(define evaluation-function (world-state-other-info state))
          ...
          ;;                 function to a _max, and you will be off to the races!
...
(check-expect (> (evaluation-function-complex test-state-3)
                 (evaluation-function-complex test-state-2))
              true)

(define (evaluation-function-complex state)
  (local [(define board (world-state-position state))]
    (count-chains (get_chains (get_perimeter_pieces board) board))))
...
(define (count-chains Loc)
  (foldr (lambda (x y)
           (cond [(= (chain-length x) 4)
                 ...
                 [else y])) ; Just for error handling... Will never actually be run
         0
         Loc))
