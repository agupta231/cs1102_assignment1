...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname perimeter_fixed) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (count-chains Loc)
  (foldr (lambda (x y) (cond [(= (chain-color x) BLACK) (+ y (sqr (* 2 (chain-length x))))] ; CPU piece
                             [(= (chain-color x) RED) (- y (sqr (* 99 (chain-length x))))] ; Player piece
                             [else y])) 0 Loc)) ; Empty Piece
